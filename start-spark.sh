...
Options:
  ...
  --ctx-slot N                  Explicit context per slot (e.g. 64k, 160k; total = slots * ctx_slot)
  --slots, -np N                Number of parallel agent slots (default: 4; up to 8 slots supported)
...
Examples:
  ...
  ./start-spark.sh --slots 4 --ctx-slot 128k  # 4 slots x 128k context (512k pool)
  ./start-spark.sh -kvu -c 512k --slots 4     # Shared dynamic unified KV pool
  ./start-spark.sh --no-thinking              # Direct fast responses (no <think> delay, temp 0.2 like Gemma)
  ./start-spark.sh --slots 1 -c 512k          # 1 slot x 512k single-agent maximized context
EOF
}
