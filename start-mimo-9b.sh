#!/usr/bin/env bash
# ==============================================================================
# start-mimo-9b.sh - Launcher for MiMo-V2.6-Distill-Qwen-9B (Xiaomi MiMo)
# Hardware: AMD Radeon RX 9060 XT (16GB VRAM, ROCm / HIP gfx1200)
# Features: Qwen3.5 9B Hybrid Attention + SFT Agentic Coding + Reasoning + Vision
# ==============================================================================
...
ALT_MODEL_Q8="${SCRIPT_DIR}/models/MiMo-V2.6-Distill-Qwen-9B-Q8_0.gguf"

DEFAULT_MMPROJ="${SCRIPT_DIR}/models/mmproj-MiMo-V2.6-Distill-Qwen-9B-bf16.gguf"
ALT_MMPROJ_F16="${SCRIPT_DIR}/models/mmproj-MiMo-V2.6-Distill-Qwen-9B-f16.gguf"

MODEL_PATH=""
MMPROJ_PATH=""
ENABLE_MMPROJ=1
ENABLE_THINKING=1
TEMPLATE_CHOICE="mimo"
ENABLE_CTX_SHIFT=1
ENABLE_KV_UNIFIED=0
...
Starts llama-server for MiMo-V2.6-Distill-Qwen-9B (Xiaomi MiMo / Qwen 3.5 Hybrid Attention)
optimized for AMD Radeon RX 9060 XT (16GB VRAM, ROCm / HIP gfx1200).

Features:
  - Supervised fine-tuned 9B agentic model for coding, cybersecurity, and tool-use
  - Hybrid linear attention with 262K native context window
  - Official Xiaomi MiMo v2.6 reasoning template (<think> tags & XML tool calls)
  - Multimodal Vision Projector support (image/vision reasoning)
  - 100% GPU Offload with Flash Attention & Q4_0 KV cache

Options:
  -m, --model PATH              Path to MiMo GGUF model (default: auto-detect Q4_K_M / Q5_K_M / Q8_0)
  ...
  --template TYPE               Chat template: mimo (default, official) | qwen (Froggeric Fixed) | native
...
Examples:
  ...
  ./start-mimo-9b.sh --slots 2 --ctx-slot 64k   # Dual-agent serving (64k context per slot)
  ./start-mimo-9b.sh -kvu -c 256k --slots 4     # Dynamic shared unified KV pool
EOF
}
...
echo -e "${BOLD}${CYAN}======================================================${NC}"
echo -e "${BOLD}${CYAN}  MiMo-V2.6-Distill-Qwen-9B Server (ROCm / HIP gfx1200)${NC}"
echo -e "${BOLD}${CYAN}======================================================${NC}"
...
if [[ -z "${MODEL_PATH}" ]]; then
    if [[ -f "${DEFAULT_MODEL}" ]]; then
        ...
        FOUND_MODELS=($(find "${SCRIPT_DIR}/models" -maxdepth 1 \( -iname "*mimo*9b*.gguf" -o -iname "*mimo-v2.6*.gguf" \) ! -iname "*mmproj*" 2>/dev/null || true))
        if [[ ${#FOUND_MODELS[@]} -gt 0 ]]; then
            MODEL_PATH="${FOUND_MODELS[0]}"
        else
            echo -e "${YELLOW}[WARN] No MiMo-V2.6-Distill-Qwen-9B GGUF model found in ./models/${NC}"
            echo -e "Run ${CYAN}./scripts/download-mimo-9b.sh${NC} to download MiMo-V2.6-Distill-Qwen-9B."
            exit 1
        fi
    fi
fi
...
if [[ "${ENABLE_MMPROJ}" -eq 1 ]]; then
    if [[ -z "${MMPROJ_PATH}" ]]; then
        ...
        elif [[ -f "${ALT_MMPROJ_F16}" ]]; then
            MMPROJ_PATH="${ALT_MMPROJ_F16}"
        else
            DETECTED_MMPROJ=($(find "${SCRIPT_DIR}/models" -maxdepth 1 \( -iname "*mimo*mmproj*.gguf" -o -iname "mmproj*mimo*.gguf" \) 2>/dev/null || true))
    ...
    if [[ -n "${MMPROJ_PATH}" && -f "${MMPROJ_PATH}" ]]; then
        ...
        MMPROJ_STATUS="Disabled (no projector found; run ./scripts/download-mimo-9b.sh mmproj)"
    fi
else
...
if [[ "${ENABLE_THINKING}" -eq 1 ]]; then
    TEMPERATURE="${CUSTOM_TEMP:-0.6}" # Official Xiaomi MiMo default
    ...
    case "${TEMPLATE_CHOICE}" in
        mimo)
            if [[ -f "${SCRIPT_DIR}/models/templates/MiMo-V2.6.jinja" ]]; then
                JINJA_ARGS+=("--chat-template-file" "${SCRIPT_DIR}/models/templates/MiMo-V2.6.jinja")
            fi
            TEMPLATE_STATUS="Official Xiaomi MiMo v2.6 (with <think> reasoning tags)"
            ;;
        qwen|froggeric)
            ...
            TEMPLATE_STATUS="Froggeric Qwen-Fixed (OpenAI JSON tool compatibility)"
            ;;
        native)
        ...
        *)
            if [[ -f "${TEMPLATE_CHOICE}" ]]; then
                ...
                echo -e "${YELLOW}[WARN] Unknown template '${TEMPLATE_CHOICE}', using official MiMo v2.6.${NC}"
                JINJA_ARGS+=("--chat-template-file" "${SCRIPT_DIR}/models/templates/MiMo-V2.6.jinja")
                TEMPLATE_STATUS="Official Xiaomi MiMo v2.6"
            fi
            ;;
    esac
    ...
    case "${TEMPLATE_CHOICE}" in
        mimo)
            if [[ -f "${SCRIPT_DIR}/models/templates/MiMo-V2.6-no-thinking.jinja" ]]; then
                JINJA_ARGS+=("--chat-template-file" "${SCRIPT_DIR}/models/templates/MiMo-V2.6-no-thinking.jinja")
            else
                JINJA_ARGS+=("--chat-template-kwargs" '{"enable_thinking": false}')
            fi
            TEMPLATE_STATUS="Xiaomi MiMo v2.6 (Pre-filled <think></think> direct output)"
            ;;
        qwen|froggeric)
        ...
        *)
            if [[ -f "${TEMPLATE_CHOICE}" ]]; then
                ...
                JINJA_ARGS+=("--chat-template-file" "${SCRIPT_DIR}/models/templates/MiMo-V2.6-no-thinking.jinja")
                TEMPLATE_STATUS="Xiaomi MiMo v2.6 (no-thinking)"
            fi
            ;;
    esac
...
echo -e "${BOLD}Architecture:${NC}        ${GREEN}Qwen3.5 9B Hybrid Linear Attention (Xiaomi SFT Distill)${NC}"
